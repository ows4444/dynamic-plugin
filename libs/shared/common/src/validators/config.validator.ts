...
export class ConfigValidator {
  ...
  static validate(config: PluginConfig, schema: Record<string, ConfigSchema>): { valid: boolean; errors: string[] } {
    const errors: string[] = [];

    this.validateObject(config, { type: 'object', properties: schema }, '', errors);
  ...
  private static validateObject(value: any, schema: ConfigSchema, path: string, errors: string[]): void {
  ...
  private static validateValue(value: any, schema: ConfigSchema, path: string, errors: string[]): void {
    if (value === null || value === undefined) {
      if (schema.required) {
  ...
  private static validateType(value: any, schema: ConfigSchema, path: string, errors: string[]): void {
  ...
  private static validateConstraints(value: any, schema: ConfigSchema, path: string, errors: string[]): void {
  ...
  private static validateStringConstraints(value: string, schema: ConfigSchema, path: string, errors: string[]): void {
    if (schema.minLength !== undefined && value.length < schema.minLength) {
      errors.push(`String at ${path} must be at least ${schema.minLength} characters long`);
    }

    if (schema.maxLength !== undefined && value.length > schema.maxLength) {
      errors.push(`String at ${path} must be at most ${schema.maxLength} characters long`);
    }

    if (schema.pattern) {
      const regex = new RegExp(schema.pattern);
      if (!regex.test(value)) {
        errors.push(`String at ${path} does not match required pattern: ${schema.pattern}`);
      }
    }
  }

  private static validateNumberConstraints(value: number, schema: ConfigSchema, path: string, errors: string[]): void {
  ...
  private static validateArrayConstraints(value: any[], schema: ConfigSchema, path: string, errors: string[]): void {
    if (schema.minLength !== undefined && value.length < schema.minLength) {
      errors.push(`Array at ${path} must have at least ${schema.minLength} items`);
    }

    if (schema.maxLength !== undefined && value.length > schema.maxLength) {
      errors.push(`Array at ${path} must have at most ${schema.maxLength} items`);
    }
  }

  private static validateArray(value: any[], schema: ConfigSchema, path: string, errors: string[]): void {
    if (!schema.items) {
      return;
    }
  ...
  static applyDefaults(config: PluginConfig, schema: Record<string, ConfigSchema>): PluginConfig {
    const result = { ...config };
    ...
    for (const [key, propertySchema] of Object.entries(schema)) {
      ...
      if (propertySchema.type === 'object' && propertySchema.properties && result[key]) {
        result[key] = this.applyDefaults(result[key], propertySchema.properties);
      }
    }
  ...
  private static inferSchemaFromValue(value: any): ConfigSchema {
    ...
    if (Array.isArray(value)) {
      const itemSchema = value.length > 0 ? this.inferSchemaFromValue(value[0]) : { type: 'string' as const };
